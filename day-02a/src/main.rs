...
score of 8 (2 because you chose Paper + 6 because you won). In the second round, your opponent will choose Paper (B), and you
should choose Rock (X). This ends in a loss for you with a score of 1 (1 + 0). The third round is a draw with both players
...
fn main() {
    ...
    results.insert("A X".to_string(), (Results::Tied, 4)); // Rock vs Rock - Tied (3+1)
    results.insert("A Y".to_string(), (Results::Won, 8)); // Rock vs Paper - Won (6+2)
    results.insert("A Z".to_string(), (Results::Lost, 3)); // Rock vs Scissors - Lost (0+3)
    results.insert("B X".to_string(), (Results::Lost, 1)); // Paper vs Rock - Lost (0+1)
    results.insert("B Y".to_string(), (Results::Tied, 5)); // Paper vs Paper - Tied (3+2)
    results.insert("B Z".to_string(), (Results::Won, 9)); // Paper vs Scissors - Won (6+3)
    results.insert("C X".to_string(), (Results::Won, 7)); // Scissors vs Rock - Won (6+1)
    results.insert("C Y".to_string(), (Results::Lost, 2)); // Scissors vs Paper - Lost (0+2)
    results.insert("C Z".to_string(), (Results::Tied, 6)); // Scissors vs Scissors - Tied (3+3)
    ...
    for (_index, raw_line) in reader.lines().enumerate() {
        ...
        let score = results.get(&line).unwrap().1;
        total_score += score;

        print!("{} ", line);
        match results.get(&line).unwrap().0 {
