...
fn main() {
    ...
    let reader = BufReader::new(File::open("../data/day-03.txt").unwrap());
    for (_index, raw_line) in reader.lines().enumerate() {
        let line: String = raw_line.unwrap().to_string();
        let half_line_len = line.len() / 2;
        let mut matching_value: u8 = 0;
        for index in line[..half_line_len].as_bytes() {
            if line[half_line_len..]
                .as_bytes()
                .iter()
                .any(|&x| x == *index)
            {
        ...
        let score = if matching_value > 96 {
            matching_value - 96
        } else {
            matching_value - 38
        };
